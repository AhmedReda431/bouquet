...
export const mutations = {
  [ADD_TO_CART](state, payload) {
    ...
    // let price = payload.price ? payload.price : 1
    ...
    if (findIndex !== -1) {
      state.data = state.data.reduce((acc, product, index) => {
        if (findIndex == index) {
          acc.push({
            ...
            qty: qty,
            // qty: product.qty + qty,
            sum: (payload.price ? payload.price : payload.product.price) * qty,
            product_size_id: payload.size,
            influncer_id: 0,
            // (product.qty + qty),
          });
        } else {
      ...
      state.data = [
        ...state.data,
        {
          ...payload.product,
          qty: qty,
          price: payload.price ? payload.price : payload.product.price,
          sum: qty * (payload.price ? payload.price : payload.product.price),
          product_size_id: payload.size,
          influncer_id: 0,  
        },
      ];
