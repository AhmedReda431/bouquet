<template>
  <form @submit.prevent="login" class="mt-5">
    ...
    <nuxt-link :to="localePath('/auth/forget')" class="forget-pass mb-3">{{
      $t("forget_password")
    }}</nuxt-link>
    <template>
...
export default {
  ...
  methods: {
    ...
    async login() {
      ...
      // } else {
      //   this.$store
      //     .dispatch("authentication/Login/LoginHandler", formData)

      //     .then(() => {
      //       if (this.$auth.user) {
      //         this.$store.dispatch("user/switchStatus", true);
      //         new swal(res.data.msg)
      //         this.$router.push("/");
      //         location.reload();
      //       }
      //     })
      ...
      try {
        ...
        this.$auth.$storage.setUniversal("type", "user");
