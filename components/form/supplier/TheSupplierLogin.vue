<template>
  <form @submit.prevent="supplierLogin" class="mt-5">
    ...
    <nuxt-link
      :to="localePath('/auth/supplier/forget')"
      class="forget-pass mb-3"
      >{{ $t("forget_password") }}</nuxt-link
    >
    <template>
      <button class="auth-btn" type="submit" v-if="!isloading">
        {{ $t("sign_in") }}
      </button>
...
export default {
  ...
  methods: {
    ...
    async supplierLogin() {
      ...
      formData.append("email_or_phone", this.form.email_or_phone);
      formData.append("password", this.form.password);
      formData.append("firebase_token", this.form.firebase_token);
      formData.append("device_type", this.form.device_type);
      ...
      try {
        ...
        this.$axios.$post(`stores/login`, formData).then((response) => {
          if (response.code == 1) {
          ...
          if (response.code == 0) {
            ...
            this.$auth.$storage.setUniversal("type", "supplier");
